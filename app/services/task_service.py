...
async def delete_task_service(session: AsyncSession, task_id: int) -> dict:
    task = await get_task_service(session, task_id)
    deleted_task =  await db_tasks.delete_db_task(session, task)
    await session.commit()
    return deleted_task

async def get_task_service(session: AsyncSession, task_id: int) -> Task:
    task = await db_tasks.get_db_task(session, task_id)
    if task is None:
        raise TaskNotFoundError(f"Task with id {task_id} not found")
    return task

async def update_task_service(session: AsyncSession, task_id: int, payload: TaskUpdate) -> Task:
